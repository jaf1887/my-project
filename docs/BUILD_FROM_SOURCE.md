...
- Owner's two Project-24 AnyKernel3 ZIPs; newer image reports `5.15.211-android13-8@MrPankaj24` and ReSukiSU `9be0f347`.
...
2. Retrieve or reconstruct the **corresponding** 5.15.211 Samsung/Project-24 source and pin a real source commit. The public 5.15.153 release is a **research baseline only**; it requires source-level vendor patch and stable-version integration before it qualifies.
...
Do not run those placeholder values: they are a **stop gate**, not a build recipe for the mismatched 5.15.153 source. If source already includes KernelSU/ReSukiSU, its existing integration must be reconciled before running the setup script (which might otherwise overwrite changes).
