...
**Build reproducibility caution:** The workflow shallow-clones the *moving* `lineage-23.2` branch, clones ReSukiSU `main`, fetches some `main` patches, and permits `patch ... || true` failures. Its release-body descriptions are assertions, not verification of every runtime feature; in particular the inspected `Image` still reports disabled Droidspaces-related cgroup/network configs. A pinned tag in the **builder repo** does not pin its external source dependencies. To reproduce Run130 exactly, capture GitHub Actions run 130 logs/provenance, source HEAD, ReSukiSU SHA, toolchain digest and patch results; then compare the published release asset's SHA-256 against the uploaded ZIP SHA-256 above. The direct asset URL is now known, but an attempted independent download on 24 September failed because the container could not resolve GitHub, and the available GitHub connector lacks a release-asset download/digest action. Thus the local owner-uploaded ZIP's SHA-256 is verified, but **a byte-for-byte match against the hosted release asset is still unverified**. To close this gap, download the URL in a network-enabled environment and run `sha256sum Project24-Kernel-m14x-ReSukiSU.zip` (or an equivalent local SHA-256 tool); it should equal the uploaded ZIP's `d9bcbc654da850a4d2e31846860fc4c4ca7403228558f00fd07d2f13da4bf067` before asserting identity.
